...
function setFrameChangedIndex (state, frame, currentIndex, newIndex) {
  console.log(`tab-moved updating from ${currentIndex} to ${newIndex}`)
  ...
  // TODO: frame.index isn't used, list order and internal index is
  frame = frame.set('index', newIndex)
  frames = frames
...
const frameReducer = (state, action, immutableAction) => {
  switch (action.actionType) {
    ...
    case appConstants.APP_TAB_MOVED: {
      console.log('tab-moved', immutableAction.toJS())
      const tabId = immutableAction.get('tabId')
